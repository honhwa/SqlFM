...
namespace SqlFM.Commands
{
    /// <summary>
    /// 格式化选中 SQL 命令处理器。
    /// 获取选中文本 → 调用 FormatService → 替换选中文本。
    /// </summary>
    internal sealed class FormatSelectedCommand
    {
        /// <summary>
        ...
        /// <summary>
        /// 命令 ID，与 VSCT 中 FormatSelectedCmdId 一致。
        /// </summary>
        ...
        /// 命令实例，确保单例。
        /// </summary>
        private static FormatSelectedCommand? _instance;
        ...
        /// 私有构造函数，防止外部实例化。
        /// </summary>
        private FormatSelectedCommand(AsyncPackage package)
        {
        ...
        public static async System.Threading.Tasks.Task InitializeAsync(AsyncPackage package)
        {
            ...
            if (commandService != null)
            {
                ...
                // 创建菜单命令，绑定执行回调
                var menuItem = CommandLocalizer.Create(menuCommandId, Execute, "CmdFormatSelected");
            ...
            // 保存单例实例
            _instance = new FormatSelectedCommand(package);
        }
        ...
        /// <summary>
        /// 命令执行回调。
        /// 获取当前选中的 SQL 文本，通过 FormatService 调用 Core 格式化引擎，然后替换选中内容。
        /// </summary>
        private static void Execute(object? sender, EventArgs e)
        {
            ...
            try
            {
                ...
                // 调用 FormatService 格式化选中文本
                var result = FormatService.FormatSelected(selectedText!);

                if (result.Success)
                {
                    // 替换选中文本为格式化后的结果
                    EditorHelper.ReplaceSelectedText(dte, result.FormattedSql);
                }
                else
                {
                    System.Diagnostics.Debug.WriteLine(
                        $"FormatSelectedCommand 格式化失败: {result.ErrorMessage}");
                }
            }
            catch (Exception ex)
            {
                // 记录异常，避免命令执行失败导致 SSMS 崩溃
                System.Diagnostics.Debug.WriteLine($"FormatSelectedCommand 执行异常: {ex.Message}");
            }
        }
    }
}
